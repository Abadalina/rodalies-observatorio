-- =============================================================================
-- 012 - Vigilar que la capa analitica siga al dia
--
-- La migracion 011 cambia el refresco entero por uno incremental guiado por una
-- marca de agua. Eso trae un modo de fallo nuevo: si el refresco dejara de
-- correr, los paneles no se vaciarian, se quedarian congelados en el ultimo dato
-- bueno. Congelado se parece demasiado a correcto.
--
-- Las ocho comprobaciones existentes vigilan la captura, que es lo irrecuperable.
-- Esta vigila la derivada, que siempre se puede rehacer con
-- `rodalies refresh --rebuild`, pero que conviene no tener rota sin saberlo.
--
-- El umbral es de tiempo, no de cantidad: con el intervalo por defecto de 15
-- minutos, una hora de retraso son cuatro pasadas perdidas. Durante una puesta
-- al dia tras una parada larga la marca va legitimamente atrasada y la
-- comprobacion avisa; es correcto que lo diga.
-- =============================================================================
